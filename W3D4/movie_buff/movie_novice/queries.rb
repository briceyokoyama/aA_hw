...
def below_average_years
  ...
  Movie.where('score < 5').group('movies.yr').select("COUNT(*) AS bad_movies, movies.yr")
end
...
def uma_movies
  ...
  Movie.joins(:actors).where(actors: {name: "Uma Thurman"}).order('movies.yr').select("movies.id, movies.title, movies.yr")
end
